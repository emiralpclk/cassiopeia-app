...
function AppContent() {
  ...
  if (error && !showApiKeyModal && !showOnboarding) {
    ...
    return (
      <div className="critical-error" style={{ padding: '40px', textAlign: 'center', background: '#0a0a0f', color: '#e8e8ed', height: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
        <h2 style={{ fontSize: '2rem', marginBottom: '16px', background: 'linear-gradient(135deg, #8ca0dc 0%, #dc8ca0 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Sistem Hatası</h2>
        <p style={{ margin: '20px 0', color: '#8a8a9a', maxWidth: '400px', lineHeight: '1.6' }}>{errorMsg}</p>
        <button onClick={() => window.location.reload()} style={{ padding: '12px 24px', background: 'linear-gradient(135deg, #8ca0dc 0%, #dc8ca0 100%)', border: 'none', borderRadius: '12px', color: '#0a0a0f', fontWeight: 'bold', cursor: 'pointer', transition: 'transform 0.2s' }}>Yeniden Dene</button>
      </div>
    );
  }
...
export default function HomePage() {
  ...
  async function loadDailyContent(card, zodiac, todayString) {
    ...
    try {
      ...
      const msg = err.message === 'API ANAHTARI_GECERSIZ' 
        ? 'API anahtarın geçersiz veya süresi dolmuş.' 
        : err.message === 'KOTA_LIMITI' 
        ? 'API kotası doldu, lütfen bekleyin.'
        : 'Bağlantı hatası veya API sorunu.';
